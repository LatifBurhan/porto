      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {portofolioData.map((item) => (
            <div
              key={item.id}
              className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg overflow-hidden hover:scale-105 transition cursor-pointer"
        ...
        >
          ...
          >
            <button
              ...
              className="absolute top-2 right-4 text-gray-500 dark:text-gray-300 text-2xl font-bold hover:text-red-500 transition"
            >
              &times;
            </button>
            <img src={selectedItem.image} alt={selectedItem.title} className="w-full h-64 object-cover rounded-xl mb-4" />
            <h3 className="text-2xl font-bold text-blue dark:text-white mb-2 transition-colors duration-300">{selectedItem.title}</h3>
            <p className="text-gray-700 dark:text-gray-300 mb-4 transition-colors duration-300">{selectedItem.description}</p>
            <button
              onClick={() => setSelectedItem(null)}
              className="bg-pink-500 text-white py-2 px-6 rounded-full hover:bg-blue-500 transition"
            >
              Tutup
            </button>
          </div>
